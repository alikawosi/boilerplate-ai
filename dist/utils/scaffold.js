...
export async function scaffoldProject(projectName, modules, withMobile = false) {
    const s = spinner();
    ...
    const infraDir = path.resolve(__dirname, "../../templates/infrastructure/supabase");
    ...
    if (withMobile) {
        ...
        if (await fs.pathExists(nativeDir)) {
            ...
            console.warn(color.yellow(`⚠ Mobile template not found at ${nativeDir}. Skipping mobile scaffold.`));
        }
    }
    ...
    if (modules.length > 0) {
        ...
        for (const module of modules) {
            ...
            if (await fs.pathExists(moduleSource)) {
                ...
                const hasNative = await fs.pathExists(path.join(moduleSource, "native"));
                // Helper function to inject files and merge env
                const inject = async (source, target, envTarget) => {
                    ...
                    if (await fs.pathExists(envSourcePath)) {
                        ...
                        if (await fs.pathExists(envTarget)) {
                            ...
                            await fs.appendFile(envTarget, `${prefix}# Module: ${module}\n${envContent}\n`);
                        }
                        else {
                ...
                if (hasWeb || hasNative) {
                    // 1. Web Injection
                    if (hasWeb) {
                        await inject(path.join(moduleSource, "web"), webTargetDir, path.join(webTargetDir, ".env.example"));
                    }
                    // 2. Native Injection
                    if (withMobile && hasNative) {
                        // Ensure native directory exists (it should, but safety first)
                        await fs.ensureDir(nativeTargetDir);
                        await inject(path.join(moduleSource, "native"), nativeTargetDir, path.join(nativeTargetDir, ".env") // Native uses .env directly
                        );
                    }
                }
                else {
                    // 3. Fallback: Legacy structure (assume web-only)
                    await inject(moduleSource, webTargetDir, path.join(webTargetDir, ".env.example"));
                }
            }
        }
...
.vercel
  `;
    await fs.writeFile(path.join(projectDir, ".gitignore"), gitignoreContent.trim());
    ...
    if (modules.includes("stripe"))
        dependenciesToInstall.push("stripe");
    if (modules.includes("resend"))
        dependenciesToInstall.push("resend");
    if (modules.includes("openai"))
        dependenciesToInstall.push("openai");
    try {
        ...
        if (dependenciesToInstall.length > 0) {
            await execa("npm", ["install", ...dependenciesToInstall, "--workspace=web"], {
                cwd: projectDir,
            });
        }
        ...
        console.error(color.yellow("⚠ Could not install dependencies automatically."));
        console.error(color.yellow(`  Please run 'npm install' and 'npm install ${dependenciesToInstall.join(" ")} --workspace=web' manually.`));
    }
    // Final Success Message
    console.log(`\n${color.bgGreen(color.black(" SUCCESS "))} Project created at ${color.green(projectDir)}`);
    console.log(`\nNext steps:`);
    console.log(`  1. ${color.cyan(`cd ${projectName}`)}`);
    console.log(`  2. ${color.cyan("cd web && cp .env.example .env.local")} (Fill in your keys)`);
    console.log(`  3. ${color.cyan("npm run dev --workspace=web")}`);
    if (withMobile) {
        console.log(`  4. ${color.cyan("cd native && npx expo start")} (To run mobile app)`);
    }
    console.log(`  ${withMobile ? "5" : "4"}. ${color.cyan("npx supabase start")} (To run local DB)`);
}
